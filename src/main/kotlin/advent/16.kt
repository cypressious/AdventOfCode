...
fun main(args: Array<String>) {
    val aunts = input.map {
        val split = it.split(", ")
        val clues = listOf(split[0].drop(split[0].indexOf(":") + 2)) + split.drop(1)
        clues.map {
            val clueSplit = it.split(": ")
            clueSplit[0] to clueSplit[1].toInt()
        }.toMap()
    }
...
private fun Map<String, Int>.eq(key: String, other: Int) = this[key]?.equals(other) ?: true
