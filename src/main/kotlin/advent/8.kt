...
fun main(args: Array<String>) {
    println(input.map { it.encode().length - it.length }.sum())
}

private fun String.encode() = buildString {
    append('"')
    this@encode.forEach {
        when (it) {
            '\\' -> append("""\\""")
...
private fun String.computeDifference() = length - drop(1).dropLast(1).memorySize()
...
private fun String.memorySize(): Int {
    ...
    while (i < length) {
        ...
        if (c != '\\') {
            ...
            if (this[i++] in escaped) {
                // do nothing
            } else {
                //hex encoded, skip 2 more
                i += 2
            }
        }
        size++
...
private val input = """"azlgxdbljwygyttzkfwuxv"
